"""
database.py — Prime Buffer SQLite Layer
...
logger = logging.getLogger("prime_buffer.database")
...
def init_db(db_path: str) -> None:
    """
    Initialize the Prime Buffer database schema.
    ...
    # OMNI Pass 3 S4 fix: add last_weekly_reset column to existing Prime Buffer DBs
    with sqlite3.connect(db_path) as conn:
        cols = {row[1] for row in conn.execute("PRAGMA table_info(circuit_breaker_state)")}
        if "last_weekly_reset" not in cols:
            conn.execute("ALTER TABLE circuit_breaker_state ADD COLUMN last_weekly_reset TEXT")
            conn.commit()
            logger.info("Migrated Prime Buffer circuit_breaker_state: added last_weekly_reset column")

    # Cipher Pass 3 P3-5: add dispatch_attempts column for Prime OMNI retry loop
    with sqlite3.connect(db_path) as conn:
...
# G8 FIX (2026-05-03): Column whitelist — same fix as alpha-buffer/database.py.
...
# ── OMNI Retry Support (Cipher Pass 3 P3-5) ──────────────────────────────────
...
def get_undispatched_concordances(
    ...
    """
    Return Prime concordances that failed OMNI dispatch and are eligible for retry.

    Cipher Pass 3 P3-5: Prime Buffer had no retry mechanism — failed dispatches
    were marked omni_dispatched=1 unconditionally and permanently lost. This
    query feeds the new _omni_retry_loop in Prime Buffer lifespan.

    Args:
        db_path:      Path to Prime database.
        older_than_s: Only return records older than this many seconds (debounce).
        max_attempts: Skip records that have been attempted this many times.
...
def increment_dispatch_attempts(
    ...
    """Increment the dispatch_attempts counter on failed Prime OMNI retry."""
    with get_conn(db_path) as conn:
        conn.execute(
            """
            UPDATE concordance_results
