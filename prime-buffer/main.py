...
def submit_pick(
    ...
    if not cb_status.can_trade:
        ...
        # P2: Alert Ahmed when Prime circuit breaker opens — deduped 30-min cooldown
        try:
            from shared.resilience.alerts import alert_ahmed as _aa
            _aa(
                f"Prime Buffer circuit breaker OPEN ({cb_status.status}).\n"
                f"Submissions blocked. Reason: {cb_status.notes}",
                key=f"prime-buffer-cb-{cb_status.status}",
                severity="WARNING",
            )
        except Exception:
            pass
