"""
circuit_breaker.py — Prime Buffer Circuit Breaker
...
logger = logging.getLogger("prime_buffer.circuit_breaker")
...
def evaluate_circuit_breaker(
    ...
    Thread-safe: holds _CB_LOCK for consistent read snapshot.
    """
    with _CB_LOCK:
        return _evaluate_locked(db_path, current_vix)
...
def _evaluate_locked(
    ...
    """Inner implementation — caller must hold _CB_LOCK."""
    ...
    if stop_triggers:
        ...
        # Cipher Pass 3 P3-13: same fix as Alpha Buffer — write manual_override=1
        # so STOP persists across daily counter resets.
...
def record_trade_outcome(
    ...
    """
    Update circuit breaker state after a trade closes. Thread-safe.
    """
    with _CB_LOCK:
        _record_outcome_locked(db_path, won, pnl_pct)
...
def _reset_daily_counters(db_path: str, today: str) -> None:
    ...
    OMNI Pass 3 S4 fix: applies the same weekly reset logic as Alpha Buffer
    (originally OMNI C1 fix). Without this, weekly_pnl_pct accumulates
    indefinitely — one bad week permanently trips CB_STOP_WEEKLY_LOSS_PCT
    and halts Prime indefinitely.

    Alpha Buffer has had this fix since OMNI Pass 1. Prime Buffer had the
    weekly reset column in the schema but the reset logic was never applied.
    """
    ...
    if last_weekly != current_week:
        ...
        # Downgrade STOP/RED status driven purely by weekly threshold
        if state.get("status") in ("STOP", "RED"):
            updates["status"] = "NORMAL"
        logger.info(
            "Prime CB weekly reset — new ISO week %s (was %s)",
