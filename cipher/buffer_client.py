...
ALSO submits directly to Railway /submit so Cipher participates in Railway
concordance (fix: Cipher was only submitting to local buffers, invisible to
...
def _submit_to_railway_async(payload: dict) -> None:
    ...
    Cipher participates in Railway concordance alongside Atlas/Sage.
...
def submit_to_alpha(
    ...
    # Compute nearest Friday in 28-38 DTE window as expiry.
    # Fetch current price from Polygon for ATM strike derivation.
    ...
    def _get_atm_strike(sym, direction):
        """Fetch prev close from Polygon and return OTM strike rounded to nearest $5."""
        try:
            ...
            if resp.status_code == 200:
                ...
                if results:
                    ...
                    # Bull put: OTM short strike ~5% below price, nearest $5
                    # Bear call: OTM short strike ~5% above price, nearest $5
    ...
    _submit_to_railway_async({
        ...
        "direction":  direction,
        "score":      score,
        "confidence": int(score),  # Explicit mapping so v2 normalization is deterministic
        "strategy":   _railway_strategy,
        "reasoning":  reasoning,
        "arena":    "alpha",
        "round":    1,         # R1 submission — single-pass scoring
        "r1_score": score,     # Mirror of score for V3 schema compatibility
        "expiry":       _expiry,
