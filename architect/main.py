"""
main.py — ARCHITECT QA Council Agent

FastAPI service — structural and architectural coherence analysis.
Port: 9014 | Brain: Anthropic Claude Sonnet 4.6
...
def _dispatch_sovereign_instruction(instr: dict) -> None:
    """Route a SOVEREIGN instruction to the appropriate handler."""
...
def _sovereign_comms_loop() -> None:
    """Background thread: poll SOVEREIGN for instructions and send heartbeats."""
    last_heartbeat: float = 0.0
    logger.info("ARCHITECT: SOVEREIGN comms loop started")
    while not _comms_stop.is_set():
        ...
        try:
            ...
            logger.warning("ARCHITECT: comms loop error: %s", e)
        _comms_stop.wait(SOVEREIGN_POLL_INTERVAL_S)
    logger.info("ARCHITECT: SOVEREIGN comms loop stopped")
...
async def lifespan(app: FastAPI):
    ...
    logger.info("ARCHITECT agent started on port %d", _settings.port)
    report(AGENT_NAME, "status", {"event": "started", "port": _settings.port}, EscalationLevel.INFO)

    _comms_stop.clear()
    threading.Thread(
        target=_sovereign_comms_loop, daemon=True, name="architect-sovereign-comms"
    ).start()
...
app = FastAPI(title="ARCHITECT QA Agent", version="1.0.0", lifespan=lifespan)


# ── Request / Response Models ─────────────────────────────────────────────────
class CodeFile(BaseModel):
    """A single source file submitted for review."""
...
class ReviewRequest(BaseModel):
    """Structural review request payload."""
...
class SovereignDirective(BaseModel):
    """Inbound SOVEREIGN directive via HTTP push."""
...
def _check_auth(provided: Optional[str]) -> None:
    """Validate X-Nexus-Secret header. Raises 403 on failure."""
    if not provided or not _secrets.compare_digest(provided, _settings.nexus_secret):
        logger.warning("ARCHITECT: unauthorized request")
...
def _run_review(
    ...
    """Execute ARCHITECT analysis in a background thread."""
    global _brain_failures_today

    if _sovereign_halted:
        logger.warning("ARCHITECT: SOVEREIGN HALT active — skipping review %s", review_cycle_id)
    ...
    if elapsed > REVIEW_TIMEOUT_S:
        logger.error("ARCHITECT: review %s TIMEOUT after %.1fs", review_cycle_id, elapsed)
    ...
    if result is None:
        ...
        logger.error("ARCHITECT: brain API failed all retries for %s", review_cycle_id)
        return
    ...
    logger.info(
        "ARCHITECT: review complete — cycle=%s service=%s assessment=%s P0=%d P1=%d",
...
async def start_review(
    ...
    """Accept a structural review request and launch async analysis."""
    ...
    logger.info(
        "ARCHITECT: review started — cycle=%s service=%s chronicle_id=%d",
...
async def health() -> JSONResponse:
    ...
    try:
        ...
        conn = _sq.connect(
            _settings.chronicle_db_path if _settings else "/dev/null", timeout=1
        )
        conn.close()
    except Exception:
        chronicle_ok = False
    ...
    overall = "healthy" if (chronicle_ok and env_ok) else "degraded"
...
async def sovereign_directive(
    ...
    logger.info("ARCHITECT: SOVEREIGN direct push: %s from=%s", d, body.from_agent)
