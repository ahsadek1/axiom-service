"""
telegram.py — Atlas Telegram Alerts + SOVEREIGN Bus Notifications

Sends critical alerts to Ahmed (Telegram) and SOVEREIGN (Nexus bus) when
Atlas encounters persistent failures.
...
def alert_brain_down(bot_token: str, chat_id: str, consecutive_failures: int) -> None:
    """
    Alert Ahmed (Telegram) and SOVEREIGN (bus) that Gemini has been down
    ...
    """
    text = (
        f"🔴 <b>Atlas Brain Down</b>\n"
        f"Gemini API has failed for {consecutive_failures} consecutive windows.\n"
        f"Atlas is not analyzing tickers. Check Google AI Studio API status."
    )
    bus_message = (
        f"[ATLAS] BRAIN DOWN — {consecutive_failures} consecutive failures. "
        f"Gemini API unreachable. Atlas not analyzing tickers."
    )
    _send(bot_token, chat_id, text)
    _bus_notify(bus_message)


def alert_submission_failed(bot_token: str, chat_id: str, ticker: str, buffer: str) -> None:
    """
    ...
    """
    text = (
        f"⚠️ <b>Atlas Submission Failed</b>\n"
        f"Failed to submit {ticker} to {buffer} Buffer after retry.\n"
        f"Check buffer service health."
    )
    bus_message = (
        f"[ATLAS] SUBMISSION FAILED — {ticker} → {buffer} Buffer after retry. "
        f"Check buffer service health."
    )
    _send(bot_token, chat_id, text)
    _bus_notify(bus_message)
