...
def _analyze_pool(payload: dict) -> None:
    ...
    try:
        # --- Deterministic scoring via atlas_scorer (replaces Gemini LLM) ---
        from atlas_scorer import score_pool as _atlas_score_pool

        valid_tickers = [t for t in pool if isinstance(t, str)]
        scored_results = _atlas_score_pool(valid_tickers, window_id=window_id)
