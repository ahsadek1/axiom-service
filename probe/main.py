...
logger = logging.getLogger("probe.main")
...
def _sovereign_comms_loop() -> None:
    ...
    logger.info("PROBE: SOVEREIGN comms loop started")
    while not _comms_stop.is_set():
        ...
        try:
            ...
            logger.warning("PROBE: comms loop error: %s", e)
        _comms_stop.wait(SOVEREIGN_POLL_INTERVAL_S)
    logger.info("PROBE: SOVEREIGN comms loop stopped")
...
async def lifespan(app: FastAPI):
    ...
    logger.info("PROBE agent started on port %d", _settings.port)
    report(AGENT_NAME, "status", {"event": "started", "port": _settings.port}, EscalationLevel.INFO)

    _comms_stop.clear()
    threading.Thread(target=_sovereign_comms_loop, daemon=True, name="probe-sovereign-comms").start()
    ...
    logger.info("PROBE agent shutting down")


app = FastAPI(title="PROBE QA Agent", version="1.0.0", lifespan=lifespan)
...
def _check_auth(provided: Optional[str]) -> None:
    if not provided or not _secrets.compare_digest(provided, _settings.nexus_secret):
        logger.warning("PROBE: unauthorized request")
...
def _run_review(
    ...
    if _sovereign_halted:
        logger.warning("PROBE: SOVEREIGN HALT active — skipping review %s", review_cycle_id)
    ...
    if elapsed > REVIEW_TIMEOUT_S:
        logger.error("PROBE: review %s TIMEOUT after %.1fs", review_cycle_id, elapsed)
    ...
    if result is None:
        ...
        logger.error("PROBE: brain API failed all retries for %s", review_cycle_id)
        return
    ...
    logger.info(
        "PROBE: review complete — cycle=%s service=%s assessment=%s P0=%d P1=%d",
...
async def start_review(
    ...
    logger.info(
        "PROBE: review started — cycle=%s service=%s chronicle_id=%d",
...
async def sovereign_directive(
    ...
    logger.info("PROBE: SOVEREIGN direct push: %s from=%s", d, body.from_agent)
