"""
chronicle_client.py — PROBE CHRONICLE Critique Bank Writer
...
logger = logging.getLogger("probe.chronicle_client")
...
def _cache_pending_create(
    ...
    try:
        ...
        conn.commit()
        conn.close()
        return -row_id  # negative = local cache ID
    except Exception as e:
...
def start_flush_thread(chronicle_db_path: str, local_db_path: str) -> None:
    ...
    with _flush_lock:
        ...
        _flush_thread = threading.Thread(target=_worker, daemon=True, name="probe-chronicle-flush")
        _flush_thread.start()
