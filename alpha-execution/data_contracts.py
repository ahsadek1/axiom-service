...
try:
    from shared.resilience.contracts import require_float as _require_float
except ImportError:
    # Fallback: inline guard so the module loads even without shared on path
    def _require_float(v, source, field, min_val=None, max_val=None):
        if v is None:
            raise DataContractError(f"{source}.{field}: missing")
        v = float(v)
        if math.isnan(v) or math.isinf(v):
            raise DataContractError(f"{source}.{field}: NaN/Inf not allowed")
        if min_val is not None and v < min_val:
            raise DataContractError(f"{source}.{field}: below {min_val}")
        if max_val is not None and v > max_val:
            raise DataContractError(f"{source}.{field}: above {max_val}")
        return v
...
class VolatilityData:
    ...
    def __post_init__(self) -> None:
        # B4: Use require_float for explicit NaN/Inf guard (math.isnan catch, not range-only)
        self.iv_rank = _require_float(
            self.iv_rank, source="volatility_data", field="iv_rank",
...
class TechnicalData:
    ...
    def __post_init__(self) -> None:
        # B4: Use require_float for explicit NaN/Inf guard
        self.rsi_14 = _require_float(
            self.rsi_14, source="technical_data", field="rsi_14",
            min_val=0.0, max_val=100.0
        )
