"""
analyzer.py — DEPLOY Brain (DeepSeek V3) Analysis Engine

Calls DeepSeek V3 API with DEPLOY's security/logic specialization prompt.
...
logger = logging.getLogger("deploy.analyzer")

AGENT_NAME = "DEPLOY"
BRAIN_MODEL = "deepseek-chat"

SYSTEM_PROMPT = (
    "You are DEPLOY, an elite deployment and operational reality specialist for live trading systems. "
    "You find missing environment variables, missing launchd plist entries, missing requirements.txt entries, "
    "service startup failure modes, log path errors, health endpoint inaccuracies, and Railway/local deploy "
    "incompatibilities. You ALWAYS return a valid JSON object with a 'findings' array. "
    "Each finding has: {severity: 'P0'|'P1'|'P2'|'P3', category: str, description: str, "
    "file: str, line_hint: str, recommendation: str}. "
    "P0 = service wont start, P1 = service starts degraded, P2 = deploy friction, P3 = missing runbook. "
    "Enumerate every deployment gap. Be thorough."
)
...
def _call_brain(api_key: str, api_url: str, user_message: str) -> Optional[dict]:
    ...
    try:
        ...
        logger.warning("DEPLOY brain API call failed: %s", e)
        return None
...
def parse_findings(brain_output: dict) -> list:
    ...
    if not isinstance(raw_findings, list):
        logger.warning("DEPLOY: brain output 'findings' is not a list")
        return []
...
def run_analysis(
    ...
    """
    Run DEPLOY analysis with 3-retry exponential backoff.
    ...
    for attempt, delay in enumerate(RETRY_DELAYS, start=1):
        logger.info("DEPLOY: brain call attempt %d/3 for %s v%s", attempt, service_name, service_version)
        ...
        if result is not None:
            ...
            logger.info(
                "DEPLOY: analysis complete — %d findings (P0=%d P1=%d P2=%d P3=%d) assessment=%s",
        ...
        if attempt < len(RETRY_DELAYS):
            logger.warning("DEPLOY: brain failure attempt %d — retrying in %.1fs", attempt, delay)
            time.sleep(delay)
        else:
            logger.error("DEPLOY: all 3 brain API attempts failed for %s v%s", service_name, service_version)
