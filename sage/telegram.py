"""
telegram.py — Sage Telegram Alerts + SOVEREIGN Bus Notifications

Sends critical alerts to Ahmed (Telegram) and SOVEREIGN (Nexus bus) when
Sage encounters persistent failures.
...
def alert_brain_down(bot_token: str, chat_id: str, consecutive_failures: int) -> None:
    """
    Alert Ahmed (Telegram) and SOVEREIGN (bus) that DeepSeek has been down
    ...
    """
    text = (
        f"🔴 <b>Sage Brain Down</b>\n"
        f"DeepSeek API has failed for {consecutive_failures} consecutive windows.\n"
        f"Sage is not analyzing tickers. Check DeepSeek API status."
    )
    bus_message = (
        f"[SAGE] BRAIN DOWN — {consecutive_failures} consecutive failures. "
        f"DeepSeek API unreachable. Sage not analyzing tickers."
    )
...
def alert_submission_failed(bot_token: str, chat_id: str, ticker: str, buffer: str) -> None:
    ...
    """
    text = (
        f"⚠️ <b>Sage Submission Failed</b>\n"
        f"Failed to submit {ticker} to {buffer} Buffer after retry.\n"
        f"Check buffer service health."
    )
    bus_message = (
        f"[SAGE] SUBMISSION FAILED — {ticker} → {buffer} Buffer after retry. "
        f"Check buffer service health."
    )
    _send(bot_token, chat_id, text)
    _bus_notify(bus_message)
