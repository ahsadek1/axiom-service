"""
config.py — Sage Agent Configuration
...
class Settings:
    """Immutable settings for the Sage agent service."""
...
def load_settings() -> Settings:
    required = {
        ...
        "DB_PATH":             os.getenv("DB_PATH", "/Users/ahmedsadek/nexus/data/sage.db"),
    }

    missing = [k for k, v in required.items() if not v]
    if missing:
        raise RuntimeError(f"Sage: missing required environment variables: {', '.join(missing)}")
    ...
    return Settings(
        ...
        deepseek_base_url=required["DEEPSEEK_BASE_URL"].rstrip("/"),
